...
Ce dossier contient tout le code frontend de l'application MakerLens.
...
| **`css/main.css`** | **Feuille de Style Principale** | Contient l'intégralité du style visuel importé des fichiers auxiliaires au dossier `css/style` (couleurs, polices, mise en page) en utilisant les variables CSS pour le thème **Vert (#00DB21)** et **Marron (#502800)**. |
| **`js/app.js`** | **Logique Frontend** | Contient le **JavaScript vanilla** pour gérer les interactions utilisateur : navigation entre les pages (vues), gestion des formulaires, affichage des tutoriels et des notifications (toasts). |
| **`assets/images/`** | **Ressources Graphiques** | Contient le logo (`logo-no-text.png`), les icônes et les autres ressources visuelles utilisées par le frontend. |

## Développement

Pour modifier l'interface, éditez les fichiers HTML, CSS ou JS. Ouvrez `index.html` dans un navigateur pour voir les changements.
...
Pour visualiser et tester l'Interface Homme-Machine localement, ouvrez le fichier **`index.html`** directement dans votre navigateur web préféré (Chrome, Firefox, Edge...).
