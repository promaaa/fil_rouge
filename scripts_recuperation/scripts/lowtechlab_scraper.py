...
SITES = [
    {
        ...
        "use_playwright": True,          # 🔁 on passe en mode dynamique
        "load_more_selector": "div.load-more",   # 🔘 bouton "Load more"
        "max_items": 200,
    },
...
async def extract_links_with_playwright(site: dict) -> list[str]:
    print(f"🧭 Mode Playwright pour {site['name']} (JS + DOM final)")
    links = []
    ...
    try:
        async with async_playwright() as p:
            ...
            # Gestion éventuelle d’un bouton "Voir plus"
            if site.get("load_more_selector"):
                ...
                while clicks_count < max_clicks:
                    try:
                        ...
                        clicks_count += 1
                        print(f"   🔁 Clic {clicks_count} sur 'voir plus'...")
    ...
    print(f"   ­ƒöù {len(uniq)} liens trouv├®s apr├¿s expansion")
    return uniq
...
async def extract_links_for_site_html(session: ClientSession, site: dict) -> list[str]:
    ...
    if not html:
        print(f"ÔØî Liste introuvable: {site['list_url']}")
        return []
...
async def scrape_tutorial_page(session: ClientSession, site: dict, url: str, out_dir: str, sem: asyncio.Semaphore):
    # 1) Calculer le slug à partir de l'URL
    ...
    # 3) Si le fichier existe déjà, on ne retélécharge pas
    if os.path.exists(out_path):
        print(f"   ↩️ déjà présent, skip : {out_path}")
        return

    # 4) Sinon, on télécharge la page normalement
    async with sem:
...
# Mots-clés dans l'URL (slug) des tutos purement "recettes"
EXCLUDED_FOOD_KEYWORDS = [
    "aliments_fermentés",      # tous les tutos "Aliments fermentés : ..."
    "boissons_fermentées",     # tous les tutos "Boissons fermentées : ..."
    "pasteurisation_de_fruits",    # "Pasteurisation de fruits et légumes"
    "conserves_lacto-fermentées",  # "Conserves lacto-fermentées"
]

# Avec les tutos "communauté / groupe / organisation" 
EXCLUDED_COMMUNITY_KEYWORDS = [
    # exemples possibles à adapter/compléter :
    "communauté_low-tech",
    "making_of",
    "low-tech_tour",
    "camp_low-tech"
...
def should_skip_tutorial(url: str) -> bool:
    """
    Retourne True si l'URL correspond à un tuto que l'on NE veut PAS récupérer.
    On se base sur le chemin de l'URL (slug) en minuscules + décodé.
    """
    path = unquote(url).lower()

    for kw in EXCLUDED_FOOD_KEYWORDS:
        if kw in path:
            return True

    for kw in EXCLUDED_COMMUNITY_KEYWORDS:
        if kw in path:
            return True
...
async def run_async():
    ...
    async with aiohttp.ClientSession(headers=HEADERS, timeout=timeout, connector=connector) as session:
        for site in SITES:
            ...
            # 🔎 Filtrage des tutos non désirés (bouffe fermentée + communauté)
            before = len(links)
            links = [u for u in links if not should_skip_tutorial(u)]
            after = len(links)
            print(f"  🧹 Filtrage MakerLens : {before} liens -> {after} liens conservés")

            # Appliquer la limite PAR SITE
            max_items = site.get("max_items", None)
            if max_items is not None:
                links = links[:max_items]
            print(f"  📌 {len(links)} liens (après limite max_items)")


            if not links:
                print("  ÔÜá´©Å Aucun lien trouv├®, passage au site suivant")
                continue
